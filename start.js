...
async function getDisplayInformation(srcToken, destToken, inputAmounts, bestPath, flag) {
  ...
  const swaps = [
    {
      name: 'FxSwap',
      price: returnAmount.dividedBy(10 ** outputDecimals).dividedBy(BN(inputAmounts).dividedBy(10 ** inputDecimals)),
  ...
  if (bitAt(flag, 0) == 1) {
    queries.push(
      uniswapv2helper.getOutputByExactInput(srcToken, destToken, inputAmounts, ADDRESS.SushiSwapFactory, 1, signer)
    );
  }
  if (bitAt(flag, 1) == 1) {
    queries.push(
      uniswapv2helper.getOutputByExactInput(srcToken, destToken, inputAmounts, ADDRESS.ShibaSwapFactory, 1, signer)
    );
  }
  if (bitAt(flag, 2) == 1) {
    queries.push(
      uniswapv2helper.getOutputByExactInput(srcToken, destToken, inputAmounts, ADDRESS.UniswapV2Factory, 1, signer)
    );
  }
  if (bitAt(flag, 3) == 1) {
    queries.push(
...
async function _queryBetweenInputAndOutput(srcToken, destToken, inputAmounts, part, flag) {
  ...
  bitAt(flag, 0) == 1
    ? queries.push(
        uniswapv2helper.getOutputByExactInput(srcToken, destToken, inputAmounts, ADDRESS.SushiSwapFactory, part, signer)
      )
    : queries.push(new Array(Number(part) + 1).fill(BN(0)));
  bitAt(flag, 1) == 1
    ? queries.push(
        uniswapv2helper.getOutputByExactInput(srcToken, destToken, inputAmounts, ADDRESS.ShibaSwapFactory, part, signer)
      )
    : queries.push(new Array(Number(part) + 1).fill(BN(0)));
  bitAt(flag, 2) == 1
    ? queries.push(
        uniswapv2helper.getOutputByExactInput(srcToken, destToken, inputAmounts, ADDRESS.UniswapV2Factory, part, signer)
      )
    : queries.push(new Array(Number(part) + 1).fill(BN(0)));
  bitAt(flag, 3) == 1
    ? queries.push(
...
app.get('/0x/quote', async (req, res) => {
  try {
    const result = await zeroEx.getQuote(req);
    if (result.code) {
